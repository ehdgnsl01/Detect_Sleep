...
VIDEO_PATH = 'data/video.mp4'  # 재생할 저장 비디오 파일 경로 (없으면 웹캠 사용)

# head down 판정 기준 (pitch 각도의 절댓값 기준)
HEAD_PITCH_THRESHOLD = 15.0  # degree
HEAD_FRAME_LIMIT = 60       # 60프레임 연속이면 졸음
...
def get_head_pose(shape, size):
    """
    dlib shape(68x2)와 이미지 크기(size=(h, w))를 받아
    head pose의 pitch, yaw, roll 각도를 구함.
    """
    image_points = np.array([
        tuple(shape[30]),    # 코끝
        tuple(shape[8]),     # 턱
        tuple(shape[36]),    # 왼쪽 눈 왼쪽 모서리
        tuple(shape[45]),    # 오른쪽 눈 오른쪽 모서리
...
def main():
    ...
    while True:
        ...
        # 웹캠일 때는 좌우 미러, 저장 비디오일 때는 전용 처리 (예: 뒤집을 필요 없거나 수직 반전)
        if is_webcam:
        ...
        for face in faces:
            ...
            # 2) Eye detection & Drowsiness logic
